...
public:
	...
	static ComPtr<IStream> getResStream(const std::wstring& resName);
	static ID2D1Factory* getD2D();
	static IDWriteFactory5* getWriteFactory();
	static ComPtr<ID2D1HwndRenderTarget> createRender(HWND& hwnd, int& w, int& h);
	...
	static ComPtr<IDWriteTextLayout> getIconLayout(const std::wstring& icon, const float& fontSize, const float& w, const float& h);
	static ComPtr<IDWriteTextLayout> getTextLayout(const std::wstring& str, const float& fontSize, const float& w, const float& h);
