...
std::unordered_map<std::wstring, std::wstring> Lang::getSupportedLang()
{
	std::unordered_map<std::wstring, std::wstring> result = { {L"简体中文",L"zh-CN"},{L"English",L"en-US"} };
	auto dataPath = Setting::get()->getDataPath();
	dataPath.append(L"Lang");
	for (const auto& entry : std::filesystem::directory_iterator(dataPath)) {
		std::wstring filename = entry.path().filename().wstring();
